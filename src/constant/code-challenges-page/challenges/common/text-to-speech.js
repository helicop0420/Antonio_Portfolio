import { Css3, ExternalLinkSquareAlt, Github, Html5, Js } from "@vicons/fa"
export default {
    title: "Text To Speech",
    des: "<p>This is a simple Text to Speech using HTML, CSS, and JS. JavaScript is fascinating with what it can do. So I made a simple, project to try and convert speech to text. I used a library called Speech Recognition.</p>",
    links: [
        {
            link: "https://github.com/BroAntonio/MyPortFolio/tree/master/text-to-speach",
            icon: Github,
            tooltip: 'Open in Github'
        },
        {
            link: "https://text-to-speach.Antonio.com",
            icon: ExternalLinkSquareAlt,
            tooltip: 'Open Link'
        }
    ],
    techs: [
        {
            name: "html",
            icon: Html5
        },
        {
            name: "javaScript",
            icon: Js
        },
        {
            name: "css",
            icon: Css3
        }
    ]
}